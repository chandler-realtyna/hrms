...
class PWANotification(Document):
	...
	def get_notification_link(self):
		...
		elif self.reference_document_type == "Timesheet Project Approval":
			return f"{base_url}/timesheets/approvals"
		elif self.reference_document_type == "Employee Invoice":
			return f"{base_url}/invoices/{self.reference_document_name}"

		return base_url


def pwa_notification_query_conditions(user=None):
	user = user or frappe.session.user
	return f"`tabPWA Notification`.`to_user` = {frappe.db.escape(user)}"


def pwa_notification_has_permission(doc, ptype=None, user=None):
	return doc.to_user == (user or frappe.session.user)
