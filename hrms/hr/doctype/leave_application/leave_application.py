...
class LeaveApplication(Document, PWANotificationsMixin):
	...
	def validate(self):
		...
		self.validate_unpaid_annual_limit()
	...
	def validate_hourly_leave(self):
		if not cint(self.is_hourly_leave):
			return

		if cint(self.half_day):
			frappe.throw(_("Hourly Leave and Half Day cannot be selected together."))

		if self.from_date and self.to_date and getdate(self.from_date) != getdate(self.to_date):
			frappe.throw(_("Hourly Leave must start and end on the same date."))

		hours = get_hourly_leave_hours(self.hourly_start_time, self.hourly_end_time)
		if hours < 1:
			frappe.throw(_("Hourly Leave must be at least 1 hour."))
		if hours > 8:
			frappe.throw(_("Hourly Leave cannot exceed 8 hours. Use a full-day request instead."))
	...
	def validate_balance_leaves(self):
		...
		if self.from_date and self.to_date:
			self.total_leave_days = get_number_of_leave_days(
				...
				self.half_day,
				self.half_day_date,
				is_hourly_leave=self.is_hourly_leave,
				hourly_start_time=self.hourly_start_time,
				hourly_end_time=self.hourly_end_time,
			)
	...
	def validate_unpaid_annual_limit(self):
		if self.leave_type != "Unpaid Leave" or not self.from_date or not self.to_date:
			return

		from hrms.hr.leave_policy_setup import get_anniversary_period

		period_start, period_end = get_anniversary_period(self.employee, self.from_date)
		if getdate(self.to_date) > period_end:
			frappe.throw(_("Unpaid Leave cannot cross the employee anniversary date."))
		...
		used = frappe.db.sql(
			...
			where employee = %(employee)s
				...
				and from_date >= %(period_start)s
				and to_date <= %(period_end)s
				and name != %(name)s
			""",
			{
				"employee": self.employee,
		...
		if flt(used) + flt(self.total_leave_days) > 10:
			frappe.throw(_("Unpaid Leave is limited to 10 days per employment year."))
	...
	def validate_leave_overlap(self):
		...
		for d in frappe.db.sql(
			...
			select
				...
				is_hourly_leave, hourly_start_time, hourly_end_time
			from `tabLeave Application`
			...
			if cint(self.is_hourly_leave) and cint(d.is_hourly_leave):
				if not hourly_ranges_overlap(
					self.hourly_start_time,
					self.hourly_end_time,
					d.hourly_start_time,
					d.hourly_end_time,
				):
					continue
...
def _time_in_seconds(value) -> int:
	...
	parts = cstr(value).split(":")
	if len(parts) < 2:
		return 0
	seconds = float(parts[2]) if len(parts) > 2 else 0
	return int(parts[0]) * 3600 + int(parts[1]) * 60 + int(seconds)


def get_hourly_leave_hours(start_time, end_time) -> float:
	if start_time is None or end_time is None:
		return 0
	start_seconds = _time_in_seconds(start_time)
	end_seconds = _time_in_seconds(end_time)
	if end_seconds <= start_seconds:
		return 0
	return flt((end_seconds - start_seconds) / 3600, 3)


def hourly_ranges_overlap(start_a, end_a, start_b, end_b) -> bool:
	return _time_in_seconds(start_a) < _time_in_seconds(end_b) and _time_in_seconds(
		start_b
	) < _time_in_seconds(end_a)
...
def get_number_of_leave_days(
	...
	is_hourly_leave: int | str | None = None,
	hourly_start_time: datetime.time | str | None = None,
	hourly_end_time: datetime.time | str | None = None,
) -> float:
	"""Returns number of leave days between 2 dates after considering half day and holidays
	(Based on the include_holiday setting in Leave Type)"""
	if cint(is_hourly_leave):
		if getdate(from_date) != getdate(to_date):
			return 0
		if not frappe.db.get_value("Leave Type", leave_type, "include_holiday") and get_holidays(
			employee, from_date, to_date
		):
			return 0
		return flt(get_hourly_leave_hours(hourly_start_time, hourly_end_time) / 8, 3)
