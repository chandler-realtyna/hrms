...
class EmployeeHoliday(Document):
	...
	def _create_holiday_list(self):
		...
		selected = {getdate(h.date): h for h in self.holidays}
		all_dates = set(_weekend_dates(int(self.year))) | set(selected)
		for date in sorted(all_dates):
			holiday = selected.get(date)
			hl.append(
				"holidays",
				{
					"holiday_date": date,
					"description": (holiday.description or "Personal Holiday") if holiday else "Weekend",
					"weekly_off": 1 if date.weekday() in (5, 6) else 0,
				},
			)

		hl.save(ignore_permissions=True)
		self._assign_holiday_list(hl_name)
	...
	def _assign_holiday_list(self, holiday_list):
		from_date = getdate(f"{self.year}-01-01")
		assignment_name = frappe.db.get_value(
			"Holiday List Assignment",
			{"assigned_to": self.employee, "from_date": from_date, "docstatus": 1},
			"name",
		)
		if assignment_name:
			frappe.db.set_value(
				"Holiday List Assignment", assignment_name, "holiday_list", holiday_list
			)
			return
