...
class EmployeeHoursReport:
	...
	def validate_standard_working_hours(self):
		...
		if not self.standard_working_hours:
			...
			self.message = _(
				"Standard Working Hours is not configured. Logged hours are accurate, but expected hours and utilization remain 0 until HR defines this policy."
			)
		else:
			self.message = None
	...
	def run(self):
		...
		return self.columns, self.data, self.message, self.chart, self.report_summary
