...
def sync_user_image_to_employee(doc, method=None) -> None:
	"""Keep the linked Employee avatar aligned with the User profile image."""
	for employee_name in frappe.get_all("Employee", filters={"user_id": doc.name}, pluck="name"):
		frappe.db.set_value("Employee", employee_name, "image", doc.user_image, update_modified=False)


@frappe.whitelist()
def upload_own_profile_image(content: str, filename: str) -> dict:
	"""Upload and set an image for the current user's own profile only."""
	import base64
	import binascii
	import io
	...
	if not current_user or current_user == "Guest":
		frappe.throw(_("You must be signed in to change your profile picture."), frappe.AuthenticationError)

	# A 5 MB binary file is at most about 6.7 MB when base64 encoded.
	if not content or len(content) > 7_000_000:
		frappe.throw(_("Profile picture must be 5 MB or smaller."))

	try:
		decoded_content = base64.b64decode(content, validate=True)
	except (binascii.Error, ValueError):
		frappe.throw(_("The selected image could not be read."))

	if len(decoded_content) > 5 * 1024 * 1024:
		frappe.throw(_("Profile picture must be 5 MB or smaller."))

	try:
		with Image.open(io.BytesIO(decoded_content)) as source_image:
			source_image.load()
			image_format = (source_image.format or "").upper()
			if image_format not in {"JPEG", "PNG", "WEBP"}:
				frappe.throw(_("Profile picture must be a JPG, PNG or WebP image."))

			if source_image.width * source_image.height > 25_000_000:
				frappe.throw(_("The selected image is too large."))
	...
	file_doc = frappe.get_doc(
		{
			...
			"folder": "Home",
			"file_name": f"profile-{frappe.generate_hash(length=12)}.{extension}",
	...
	for employee_name in frappe.get_all("Employee", filters={"user_id": current_user}, pluck="name"):
		frappe.db.set_value("Employee", employee_name, "image", file_doc.file_url, update_modified=False)
...
def search_employee_projects(
	...
	**kwargs,
	...
	"""Return all available projects, prioritizing relevant ones for the employee."""
	employee = get_current_employee_info()
	first_name = (employee.get("first_name") if employee else "") or ""
	employee_id = (employee.get("name") if employee else "") or ""
	can_view_all_projects = frappe.session.user == "babak@realtyna.net"

	def normalize_search_text(value: str) -> str:
		return " ".join(part for part in re.split(r"[\W_]+", (value or "").casefold()) if part)
	...
	def matches_search(project: frappe._dict) -> bool:
		if not normalized_query:
			return True
		for value in (project.name, project.project_name):
			normalized_value = normalize_search_text(value)
			if normalized_query in normalized_value or compact_query in normalized_value.replace(" ", ""):
				return True
		return False

	def is_available(project: frappe._dict) -> bool:
		if can_view_all_projects:
			return True
		project_name = project.project_name or ""
		if not project_name.casefold().startswith(SPECIAL_TASKS_PROJECT_PREFIX.casefold()):
			return True
		assignee = project_name[len(SPECIAL_TASKS_PROJECT_PREFIX) :].strip()
		return bool(first_name) and assignee.casefold() == first_name.strip().casefold()
	...
	recent_projects = (
		frappe.db.sql_list(
			...
			INNER JOIN `tabTimesheet` timesheet ON timesheet.name = detail.parent
			WHERE timesheet.employee = %s
				AND COALESCE(detail.project, '') != ''
			GROUP BY detail.project
			ORDER BY MAX(COALESCE(detail.to_time, detail.from_time, timesheet.modified)) DESC
			""",
			employee_id,
		)
		if employee_id
	...
	recent_project_rank = {project: index for index, project in enumerate(recent_projects)}

	def project_sort_key(project: frappe._dict) -> tuple[int, int, str]:
		is_responsible = bool(employee_id) and project.custom_project_lead == employee_id
		return (
			0 if is_responsible else 1,
			recent_project_rank.get(project.name, len(recent_project_rank)),
			normalize_search_text(project.project_name or project.name),
		)

	available_projects = sorted(
		(project for project in projects if is_available(project) and matches_search(project)),
		key=project_sort_key,
	)
	start = cint(start)
	# The mobile autocomplete does not paginate its popover, so return the full
	# remaining list instead of silently hiding projects after the first page.
	return [
		(project.name, project.project_name)
		for project in available_projects[start:]
	]
...
def mark_notifications_as_read(names: list[str] | str | None = None) -> int:
	if isinstance(names, str):
		names = frappe.parse_json(names)
	if names is None:
		names = []
	if not isinstance(names, list):
		frappe.throw(_("Invalid notification list."))

	unique_names = list(dict.fromkeys(str(name) for name in names if name))[:100]
	if unique_names:
		frappe.db.set_value(
			"PWA Notification",
			{
...
def get_leave_types(employee: str, date: str) -> list:
	from hrms.hr.doctype.leave_application.leave_application import get_leave_details
	from hrms.hr.leave_policy_setup import VISIBLE_LEAVE_TYPES

	date = date or getdate()

	# Get leave details validate leave access internally
	leave_details = get_leave_details(employee, date)
	available = set(leave_details["leave_allocation"].keys()) | set(leave_details["lwps"])
	leave_types = [leave_type for leave_type in VISIBLE_LEAVE_TYPES if leave_type in available]
...
@frappe.whitelist()
def notify_long_running_timer(employee: str, project: str, started_at: str) -> bool:
	"""Notify the employee when an active browser timer reaches two hours."""
