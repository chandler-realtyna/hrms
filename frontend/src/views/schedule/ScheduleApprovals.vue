<template>
	<ion-page>
		...
		<ion-content :fullscreen="true">
			...
			>
				<FeatherIcon name="calendar" class="w-12 h-12 text-gray-300" />
				<p class="text-base font-medium">{{ __("No pending approvals") }}</p>
				<p class="text-sm">{{ __("All schedule requests have been reviewed.") }}</p>
			</div>
			...
			<div v-else class="p-4 space-y-3">
				<div
					...
					:key="item.name"
					class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden cursor-pointer active:bg-gray-50"
					...
					<div class="p-4 flex items-center gap-3">
						...
						<div class="flex-1 min-w-0">
							...
							<p class="text-xs text-gray-500 mt-0.5">
								{{ item.employee }} · {{ item.year }} · {{ item.timezone }}
							</p>
						</div>
						<!-- Badge -->
						<span class="flex-shrink-0 text-xs font-semibold px-2.5 py-1 rounded-full bg-yellow-100 text-yellow-700">
							{{ __("Pending") }}
						</span>
...
const approvalsResource = createResource({
	url: "hrms.api.get_pending_schedule_approvals",
	onError() {},
})

function goToDetail(name) {
	router.push({ name: "ScheduleApprovalDetail", params: { id: name } })
}
...
onMounted(() => { approvalsResource.reload() })
</script>
