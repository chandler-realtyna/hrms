...
const formFields = createResource({
	...
	transform(data) {
		...
		return fields.map((field) => {
			...
			if (["hourly_start_time", "hourly_end_time"].includes(field.fieldname)) {
				field.hidden = true
			}
...
watch(
	...
	(half_day) => {
		if (half_day) leaveApplication.value.is_hourly_leave = 0
		setHalfDayDate(half_day)
	}
)

watch(
	() => leaveApplication.value.is_hourly_leave,
	(is_hourly_leave) => {
		if (is_hourly_leave) {
			leaveApplication.value.half_day = 0
			leaveApplication.value.to_date = leaveApplication.value.from_date
		}
		setHourlyFields(is_hourly_leave)
		setTotalLeaveDays()
	}
)

watch(
	() => [leaveApplication.value.hourly_start_time, leaveApplication.value.hourly_end_time],
	() => setTotalLeaveDays()
)
...
function setTotalLeaveDays() {
	...
	const leaveDays = createResource({
		...
		params: {
			...
			is_hourly_leave: leaveApplication.value.is_hourly_leave,
			hourly_start_time: leaveApplication.value.hourly_start_time,
			hourly_end_time: leaveApplication.value.hourly_end_time,
		},
		onSuccess(data) {
...
function setHourlyFields(is_hourly_leave) {
	for (const fieldname of ["hourly_start_time", "hourly_end_time"]) {
		const field = formFields.data.find((item) => item.fieldname === fieldname)
		if (!field) continue
		field.hidden = !is_hourly_leave
		field.reqd = !!is_hourly_leave
	}
}
...
function validateForm() {
	...
	setHourlyFields(leaveApplication.value.is_hourly_leave)
