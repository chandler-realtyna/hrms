<template>
	<div>
		<!-- Not connected state -->
		<div v-if="!calendarStatus.loading && !calendarStatus.data?.is_connected" class="flex flex-col items-center justify-center h-full gap-5 px-6 py-10 text-center">
...
// Pre-populate with the current user's employee record plus any
// employees passed in (e.g. selected on the Team Availability page).
onMounted(() => {
	const picked = new Map()
	const emp = currentEmployee?.data
	if (emp?.name && emp?.employee_name) {
		picked.set(emp.name, { name: emp.name, employee_name: emp.employee_name, designation: emp.designation || "" })
	}
	for (const e of props.initialEmployees || []) {
		if (e?.name && !picked.has(e.name)) {
			picked.set(e.name, { name: e.name, employee_name: e.employee_name || e.name, designation: e.designation || "" })
		}
	}
	selectedEmployees.value = [...picked.values()]
