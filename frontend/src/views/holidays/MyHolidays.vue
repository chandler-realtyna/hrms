<template>
	<ion-page>
		...
		<ion-content :fullscreen="true">
			...
			<div v-if="!initialized || holidayRecord.loading" class="flex items-center justify-center h-48">
			...
			<div v-else-if="initialized" class="pb-10">
				...
				<div
					v-if="isEditable && selectedDates.length > 0"
					class="mx-4 mt-5"
				>
				...
				<div v-if="isEditable" class="mx-4 mt-6 flex flex-col gap-3">
					...
					>
						...
						<span v-else>
							{{ 15 - selectedDates.length }} {{ __("more day(s) needed") }}
						</span>
					</button>
				</div>
			</div>
...
const currentMonthLabel = computed(() =>
	currentViewMonth.value.format("MMMM YYYY")
)
...
const holidayRecord = createResource({
	...
	onSuccess(data) {
		...
		if (data && data.holidays) {
			...
			selectedDates.value = data.holidays.map((h) =>
