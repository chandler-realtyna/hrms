...
import { arePushNotificationsEnabled, unreadNotificationsCount } from "@/data/notifications"
...
if (session?.isLoggedIn) {
	...
	arePushNotificationsEnabled.reload().catch(() => {})
}
...
const registerServiceWorker = async () => {
	...
	if ("serviceWorker" in navigator) {
		...
		if (window.frappe?.boot?.push_relay_server_url && session?.isLoggedIn) {
			try {
				...
				console.error("Failed to check push notification availability", err)
			}
		}
